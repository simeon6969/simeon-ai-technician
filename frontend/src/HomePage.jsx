...
export default function HomePage({ loggedIn, onEnter, onRegister }) {
  ...
  useEffect(() => {
    ...
    getPublicSaleItems(query, offset, controller.signal).then((data) => {
      ...
      setItems((previous) => offset ? [...previous, ...data.items.filter((item) => !previous.some((old) => old.item_id === item.item_id))] : data.items)
      setTotal(data.total)
      setError(false)
    }).catch((failure) => {
  ...
  return <div id="home" className="min-h-screen bg-[#f8faf9] text-slate-900">
    ...
    <main>
      ...
      <section id="sales-board" className="mx-auto max-w-7xl scroll-mt-8 px-6 py-16">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">{items.map((item) => <article key={item.item_id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white transition hover:shadow-lg hover:shadow-slate-200/50"><div className="aspect-[4/3] bg-slate-100"><img src={item.photo_data} alt={item.name} loading="lazy" className="h-full w-full object-contain p-3" /></div><div className="p-5"><p className="text-xs text-slate-500">{c.seller} {item.seller_name}</p><h3 className="mt-2 break-words text-lg font-semibold">{item.name}</h3><p className="mt-3 text-xl font-bold text-teal-800">{new Intl.NumberFormat(language, { style: 'currency', currency: item.currency }).format(Number(item.price))}</p><p id={`listing-${item.item_id}`} className={`mt-4 whitespace-pre-wrap break-words text-sm leading-relaxed text-slate-600 ${expanded === item.item_id ? '' : 'line-clamp-2'}`}>{item.description}</p><button aria-expanded={expanded === item.item_id} aria-controls={`listing-${item.item_id}`} onClick={() => setExpanded(expanded === item.item_id ? null : item.item_id)} className="mt-4 text-sm font-semibold text-teal-700 hover:underline">{expanded === item.item_id ? c.close : c.details} →</button></div></article>)}</div>
