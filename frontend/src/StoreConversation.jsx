    ...
    <main className="mx-auto max-w-3xl px-4 py-6">
      ...
      <div ref={bottom} className="mt-6 rounded-2xl bg-white p-5 shadow-sm" aria-live="polite">
        {saved ? <><p className="font-semibold text-green-700">{w[10]} #{saved}</p>{validationFailed && <p className="mt-3 text-amber-800">{w[13]}</p>}</> : review ? <><p className="mb-4">{w[5]}</p><button disabled={busy} onClick={save} className="rounded-xl bg-slate-900 px-5 py-3 text-white disabled:opacity-50">{t(busy ? 'Saving...' : kind === 'job' ? 'Save Job Card' : 'Save Spare Part')}</button></> : <>
          ...
          {field[4] === 'boolean' ? <div className="mt-4 flex flex-wrap gap-3">{[true, false].map((value) => <button key={String(value)} onClick={() => reply(value)} className="rounded-xl border border-slate-300 px-4 py-3">{value ? w[8] : w[9]}</button>)}</div> : field[4] === 'availability' ? <div className="mt-4 flex flex-wrap gap-3">{['available', 'limited', 'unavailable', 'unknown'].map((value) => <button key={value} onClick={() => reply(value)} className="rounded-xl border border-slate-300 px-4 py-3">{t(value)}</button>)}</div> : field[4] === 'photo' ? <input aria-label={t(field[1])} disabled={busy} type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => photo(event.target.files[0])} className="my-4 block w-full" /> : <form onSubmit={(event) => { event.preventDefault(); reply(draft.trim()) }}><textarea ref={input} aria-label={t(field[1])} value={draft} onChange={(event) => setDraft(event.target.value)} rows={3} maxLength={12000} className="mt-4 w-full rounded-xl border border-slate-300 p-3" /><button className="mt-3 rounded-xl bg-slate-900 px-5 py-3 text-white">{w[3]}</button></form>}
